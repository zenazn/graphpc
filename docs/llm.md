...
- Sessions are split into epochs (connection-scoped cache lifetime).
- Reads coalesce/cache within an epoch.
- `@method` calls never coalesce.
- Hydration epoch is short-lived and payload-backed.
- Reconnect starts a fresh epoch.

Details: [Epochs and Caching](caching.md), [SSR and Hydration](ssr-and-hydration.md), [Reconnection](reconnection.md).
...
Built-ins extend `RpcError`: `ValidationError`, `EdgeNotFoundError`, `MethodNotFoundError`, `ConnectionLostError`.
...
- Set limits: `maxTokens`, `maxPendingOps`, `maxQueuedOps`, payload size.
