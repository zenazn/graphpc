...
Two kinds of thrown values survive the round-trip as actual class instances: `RpcError` and its built-in subclasses, and registered custom types. (A custom `RpcError` subclass _without_ a registered reducer is not wrapped — it arrives as a base `RpcError` with its code and message preserved, but `instanceof` the subclass fails.) Any other thrown value — including built-ins like `Date` or `Map`, which pass through fine as _returned_ data but not as errors — is wrapped in an `RpcError` with a code indicating the operation type (`EDGE_ERROR`, `GET_ERROR`, `DATA_ERROR`, or `STREAM_ERROR`). The thrown value's string form becomes the wrapped message (unless [redaction](production.md#error-redaction) — on by default outside `NODE_ENV` `"development"`/`"test"` — replaces it with `"Internal server error"`); `instanceof CustomError` will fail on the client, and any structured fields are lost.
